...
$HostsDiskLatency = @()
foreach ($VMHost in (Get-vCheckvSphereObject "Hosts" | ?{$_.ConnectionState -eq "Connected"})) {
...
$Header = ("Disk Max Total Latency over {0}ms: {1}" -f $diskmaxtotallatency, @($HostsDiskLatency).count)
...
Remove-Variable $HostsDiskLatency